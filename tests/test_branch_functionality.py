...
def test_without_stored_config(tmp_path):
    ...
    assert json.load(open(tmp_path / ".auto-track" / "data_branches.json", 'r')) == {"test_func": {"{'some_param': 'some_value'}": "test_branch"}}
    ...
    at_config["some_param"] = "another_value"
    assert get_data_branch(at_config, tmp_path, 'test_func') == "some_branch"
    assert json.load(open(tmp_path / ".auto-track" / "data_branches.json", 'r')) == {"test_func": {"{'some_param': 'some_value'}": "test_branch", "{'some_param': 'another_value'}": "some_branch"}}

    assert get_data_branch(None, tmp_path, 'test_func') == "main"

    assert get_data_branch(at_config, tmp_path, 'another_func') == "some_branch"
    assert json.load(open(tmp_path / ".auto-track" / "data_branches.json", 'r')) == {"test_func": {"{'some_param': 'some_value'}": "test_branch", "{'some_param': 'another_value'}": "some_branch"}, "another_func": {"{'some_param': 'another_value'}": "some_branch"}}